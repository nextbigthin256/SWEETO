...
export function attachAdminCategoriesListeners(context, shadow) {
  ...
  if (dropzone && fileInput) {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      if (file) {
        window.dispatchEvent(new CustomEvent('toast:show', { detail: 'Uploading category image to Supabase Cloud Storage...' }));
        try {
          ...
          window.dispatchEvent(new CustomEvent('toast:show', { detail: cloudUrl ? 'Category image saved to Supabase Cloud Storage! ☁️' : 'Image loaded!' }));
        } catch(err) {
          const reader = new FileReader();
          reader.onload = (e) => {
