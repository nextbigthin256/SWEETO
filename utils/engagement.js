...
export function isUserEngaged() {
  ...
  const views = parseInt(localStorage.getItem('SWEETOS_page_views') || '0');
  return views >= 3;
}

export function incrementPageView() {
  let views = parseInt(localStorage.getItem('SWEETOS_page_views') || '0');
  views++;
  localStorage.setItem('SWEETOS_page_views', String(views));
